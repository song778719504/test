...
public class PayService implements IPayService {
    ...
    public ServerResponse trade_precreate(OrderVO orderVO) {
        ...
        switch (result.getTradeStatus()) {
            ...
            case FAILED:
                log.error("支付宝预下单失败!!!");
                return ServerResponse.serverResponseByFail(StatusEnum.ALIPAY_ORDER_FAIL.getStatus(),StatusEnum.ALIPAY_ORDER_FAIL.getMsg());
    ...
    public String callBackLogic(Map<String, String> signParam) {
        ...
        OrderVO orderVO = serverResponse.getData();
        if (orderVO.getStatus() >= Consts.OrderStatusEnum.PAYEN.getStatus()){
            return "success";
        }
        ...
        if ("TRADE_SUCCESS".equals(tradeStatus)){//订单支付成功
            //修改订单状态、修改订单支付时间
            ServerResponse serverResponse1 = orderService.updateOrderStatus(orderNo,signParam.get("gmt_payment"),Consts.OrderStatusEnum.PAYEN.getStatus());
        ...
        payInfo.setPayPlatform(Consts.PaymentPlatform.ALIPAY.getCheck());
